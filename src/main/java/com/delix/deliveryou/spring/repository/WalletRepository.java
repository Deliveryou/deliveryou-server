...
public interface WalletRepository  extends JpaRepository<Wallet, Long> {
    ...
    @Modifying
    @Transactional
    int increaseWalletCredit(@Param("walletId") long walletId, @Param("amount") int amount);

    @Query("update Wallet set credit = credit - :amount where id = :walletId")
    @Modifying
    @Transactional
    int decreaseWalletCredit(@Param("walletId") long walletId, @Param("amount") int amount);

    @Query("select case when w.credit - :amount >= 0 then true else false end from Wallet w where w.id = :walletId")
    @Transactional
    boolean canDecreaseWalletCredit(@Param("walletId") long walletId, @Param("amount") int amount);
